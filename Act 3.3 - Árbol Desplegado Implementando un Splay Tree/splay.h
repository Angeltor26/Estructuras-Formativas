...
Node<T>* Node<T>::add(T val){
  if(val>value){
    if(right != 0){return right -> add(val);}
    else{right = new Node(val, this);}
    return right;
  }
  else{
    if(left != 0){return left -> add(val);} else{left = new Node(val, this);return left;}
  }
}
...
Node<T>* Node<T>::find(T val){
  ...
  else if(left !=0){return left->find(val);}else{return 0;}
}
...
Node<T>* Node<T>::rot_right(Node<T>* y){
    ...
    y->parent = x;
    if (x->parent){
      if ((x->parent->right) && (x->parent->value < x->value)){x->parent->right = x;}
...
Node<T>* Node<T>::rot_left(Node<T>* x){
  ...
  x->parent = y;
  if(y->parent){
    if((y->parent->left)&&(y->parent->value > y->value)){y->parent->left = y;}
...
Node<T>* Node<T>::splay(Node<T> *rt, Node<T> *x){
    while(x->parent != 0){
      if(rt->value == x->parent->value){
        if(x->parent->left && x->parent->left->value == x->value){rot_right(rt)};
        else{rot_left(rt);}
      }
      else{
        Node<T>*p = x->parent; 
...
void SplayTree<T>::add(T val){
  if(root==0){root = new Node<T>(val);}
  else{Node<T> *added = root -> add(val);root = root->splay(root, added);}
}
